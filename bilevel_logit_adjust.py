...
parser.add_argument('--ARCH_END', dest='ARCH_END', default=300, type=int)
parser.add_argument('--ARCH_INTERVAL', dest='ARCH_INTERVAL', default=1, type=int)
...
val_lr_scheduler=optim.lr_scheduler.MultiStepLR(val_optimizer,milestones=[160,180],gamma=0.1)
...
for i in range(args.checkpoint+1,total_epoch+1):
        ...
        text,loss,train_acc=eval_epoch(eval_train_loader,model,cross_entropy,i,' train_dataset',params=[dy,ly],logit_adjust=logit_adjust_ly,num_classes=num_classes)
        logfile.write(text+'\n')
        text,loss,val_acc=eval_epoch(val_loader,model,cross_entropy,i,' val_dataset',params=[dy,ly,wy],logit_adjust=logit_adjust_ly,num_classes=num_classes)
        logfile.write(text+'\n')
        text,loss,test_acc=eval_epoch(test_loader,model,cross_entropy,i,' test_dataset',params=[dy,ly,wy],logit_adjust=logit_adjust_ly,num_classes=num_classes)
        logfile.write(text+'\n')
        print(dy,ly,'\n')
        train_epoch(i, model, 
